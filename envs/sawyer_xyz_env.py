...
from envs.mujoco_env import MujocoEnv, _assert_task_is_set
...
class SawyerXYZEnv(SawyerMocapBase):
  ...
  def __init__(
    ...
    Args:
      ...
      random_init: Whether to randomly initialize in reset_model()
      get_image_obs: Whether to return image observations
      image_obs_size: Image observation resolution
      camera_name: Image observation viewpoint
      frame_skip: Frame skip
      hand_low: Range of hand position
      hand_high: Range of hand position
      mocap_low: Range of mocap
      mocap_high: Range of mocap
      action_scale: Action scale
      action_rot_scale: Action rotation scale
    """
    ...
    # Image observation viewpoint
    self._get_image_obs = get_image_obs
    self._image_obs_size = image_obs_size
  ...
  def _get_obs(self):
    ...
    if self._get_image_obs:
      img_obs = self.render(
          offscreen=True, camera_name=self._camera_name,
          resolution=self._image_obs_size)
      img_obs = img_obs.transpose(2, 0, 1)
    else:
      img_obs = None
  ...
  def observation_space(self):
    ...
    proprio_space = Box(
        ...
        low=np.hstack((self._HAND_SPACE.low, obj_low, goal_low)),
        high=np.hstack((self._HAND_SPACE.high, obj_high, goal_high)),
        dtype=np.float32,
    )

    if self._get_image_obs:
      image_space = Box(low=0, high=256, shape=(3, 84, 84), dtype=np.uint8)
      return Dict({
          "image": image_space,
  ...
  def step(self, action):
    ...
    reward, info = self.evaluate_state(
        self._last_stable_obs['proprio'], action)
    done = info['success'] or self.curr_path_length >= self.max_path_length
    return self._last_stable_obs, info['success'], done, info
