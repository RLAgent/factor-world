...
class SawyerSoccerEnvV2(SawyerXYZEnv):
    MODEL_NAME = full_v2_path_for('sawyer_xyz/sawyer_soccer.xml')
    OBJ_RADIUS = 0.013
    TARGET_RADIUS=0.07

    def __init__(self, model_name: str = MODEL_NAME,
                 random_obj_pos: bool = True,
                 random_goal: bool = False,
                 **kwargs):
        """
        Args:
          random_init_pos: Randomize initial object position in reset_model() if
              self.random_init is True
          random_goal: Randomize goal position in reset_model() if
              self.random_init is True
        """
        self.random_obj_pos = random_obj_pos
    ...
    def reset_model(self):
        ...
        if self.random_init:
            ...
            if self.random_goal:
                rand_state = self._get_state_rand_vec()
                self._target_pos = rand_state[3:]
                self.sim.model.body_pos[self.model.body_name2id('goal_whole')] = self._target_pos

            # Randomize object init position.
            if self.random_obj_pos:
                # Sample obj_init_pos until it's sufficiently far from target_pos.
                rand_state = self._get_state_rand_vec()
                while np.linalg.norm(rand_state[:2] - self._target_pos[:2]) < 0.15:
                    rand_state = self._get_state_rand_vec()

                self.obj_init_pos = np.concatenate((rand_state[:2], [self.obj_init_pos[-1]]))
