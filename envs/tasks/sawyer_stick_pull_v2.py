...
class SawyerStickPullEnvV2(SawyerXYZEnv):
  ...
  def reset_model(self):
    ...
    self.obj_init_qpos = np.array([0., 0.09])
    self.obj_init_pos = self.init_config['obj_init_pos']
    self._target_pos = np.array([0.3, 0.4, self.obj_init_pos[-1]])
  ...
  def compute_reward(self, action, obs):
    ...
    stick_in_place_margin = (np.linalg.norm(
        (self.obj_init_pos - container_init_pos) * yz_scaling))
    ...
    stick_in_place_margin_2 = np.linalg.norm(self.obj_init_pos - target)
    ...
    grasp_success = (tcp_to_stick < 0.02 and (tcp_opened > 0)
                     and (stick[2] - 0.01 > self.obj_init_pos[2]))
