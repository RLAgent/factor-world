...
class SawyerStickPushEnvV2(SawyerXYZEnv):
  ...
  def __init__(self, model_name=MODEL_NAME, **kwargs):
    ...
    self.init_config = {
        'obj_init_pos': np.array([-0.1, 0.6, 0.02]),
        'hand_init_pos': np.array([0, 0.6, 0.2]),
    }
    self.goal = self.init_config['obj_init_pos']
  ...
  def evaluate_state(self, obs, action):
    stick = obs[4:7]
    container = obs[11:14]
    reward, tcp_to_obj, tcp_open, container_to_target, grasp_reward, stick_in_place = self.compute_reward(
        action, obs)
    success = float(np.linalg.norm(container - self._target_pos) <= 0.12)
    near_object = float(tcp_to_obj <= 0.03)
    grasp_success = float(self.touching_object and (
        tcp_open > 0) and (stick[2] - 0.01 > self.obj_init_pos[2]))
  ...
  def reset_model(self):
    self._reset_hand()
    self.obj_init_pos = self.init_config['obj_init_pos']
    self._target_pos = np.array([0.4, 0.6, self.obj_init_pos[-1]])
    ...
    self._set_stick_xyz(self.obj_init_pos)
  ...
  def compute_reward(self, action, obs):
    ...
    stick_in_place_margin = (np.linalg.norm(
        self.obj_init_pos - target)) - _TARGET_RADIUS
    ...
    container_in_place_margin = np.linalg.norm(
        self.obj_init_pos - target) - _TARGET_RADIUS
