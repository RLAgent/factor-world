...
class SawyerPickPlaceEnvV2(SawyerXYZEnv):
  ...
  """
  MODEL_NAME = full_v2_path_for('sawyer_xyz/sawyer_pick_place_v2.xml')
  ...
  def __init__(self, model_name=MODEL_NAME, **kwargs):
    ...
    self.goal = np.array([-0.005, 0.4, 0.02])
  ...
  def evaluate_state(self, obs, action):
    ...
    reward, tcp_to_obj, tcp_open, obj_to_target, grasp_reward, in_place_reward = self.compute_reward(
        action, obs)
    success = float(obj_to_target <= 0.03)
    near_object = float(tcp_to_obj <= 0.03)
    grasp_success = float(self.touching_main_object and (
        tcp_open > 0) and (obj[2] - 0.02 > self.obj_init_pos[2]))
    info = {
        'success': success,
  ...
  def reset_model(self):
    ...
    # Add a small amount of noise to object position
    noise = np.random.uniform(
        low=(-0.03, -0.03, 0.0),
        high=(0.03, 0.03, 0.0),
    )
    self._set_obj_xyz(self.obj_init_pos + noise)
    self.num_resets += 1
  ...
  def _gripper_caging_reward(self, action, obj_position):
    ...
    obj_position_x_z = np.copy(obj_position) + \
        np.array([0., -obj_position[1], 0.])
    tcp_obj_norm_x_z = np.linalg.norm(tcp_xz - obj_position_x_z, ord=2)

    # used for computing the tcp to object object margin in the x_z plane
    init_obj_x_z = self.obj_init_pos + \
        np.array([0., -self.obj_init_pos[1], 0.])
    init_tcp_x_z = self.init_tcp + np.array([0., -self.init_tcp[1], 0.])
    tcp_obj_x_z_margin = np.linalg.norm(
        init_obj_x_z - init_tcp_x_z, ord=2) - x_z_success_margin
